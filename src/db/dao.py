...
from src.db.model import Tariff
...
class BaseDAO(Generic[ModelType]):
    ...
    @classmethod
    async def delete_filter(cls, **filter_by) -> None:
        filters = {k: v for k, v in filter_by.items() if v is not None}
        async with session() as sess:
            try:
                query = delete(cls.model.__table__).filter_by(**filters)
                await sess.execute(query)
                await sess.commit()
            except IntegrityError:
                await sess.rollback()
...
class TariffDAO(BaseDAO[Tariff]):
    model = Tariff

    @classmethod
    async def select_latest_tariff(
        cls, cargo_type: str = ''
    ) -> ModelType | None:
        async with session() as sess:
            try:
                query = (
                    select(cls.model)
                    .filter_by(cargo_type=cargo_type)
